...
const { sendTestRequestEmail, sendTestAdmissionEmail, sendAppointmentEmail, sendReportReadyEmail } = require('../services/emailService');
...
const createTestRequest = async (req, res) => {
  try {
    ...
    try {
      ...
      savedRequest.emailNotifications.requestSent = true;
  ...
  try {
    ...
    try {
      if (status === 'Admitted') {
        ...
        updatedRequest.emailNotifications.requestAdmitted = true;
        ...
        updatedRequest.emailNotifications.appointmentScheduled = true;
      }
...
const uploadTestReport = async (req, res) => {
  try {
    ...
    try {
      ...
      updatedRequest.emailNotifications.reportReady = true;
      await updatedRequest.save();
    } catch (emailError) {
      console.error('Failed to send email notification:', emailError);
    }
